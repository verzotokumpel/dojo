...
fn test_pack_multiple() {
    ...
    assert(
        *packed.at(0) == 188719626707717088982296698380167795313645871959412740063448560304128, 
        'Packed multiple 0'
    );
    assert(
        *packed.at(1) == 12940774403044448679501384905228180427841125450824163620418038790095104, 
        'Packed multiple 1'
    );
    assert(
        *packed.at(2) == 1541463130217537339061372343828480, 
        'Packed multiple 2'
    );
}
...
fn test_pack_unpack_types() {
    ...
    pack_inner(@58, 252, ref packing, ref offset, ref packed); 
    ...
    pack_inner(@contract_address.into(), 252, ref packing, ref offset, ref packed);  
    let class_hash = Felt252TryIntoClassHash::try_into(1337).unwrap();
    pack_inner(@class_hash.into(), 252, ref packing, ref offset, ref packed);  
    ...
    assert(
        unpack_inner(252, ref packed_span, ref unpacking, ref un_offset).unwrap().try_into().unwrap()
        == contract_address, 
